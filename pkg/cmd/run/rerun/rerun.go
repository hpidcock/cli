...
func runRerun(opts *RerunOptions) error {
	...
	if opts.Prompt {
		...
		runs, err := shared.GetRunsByStatus(client, repo, 50, shared.Completed)
		if err != nil {
			return fmt.Errorf("failed to get runs: %w", err)
		}
		filtered := []shared.Run{}
		for _, run := range runs {
			if run.Conclusion != shared.Success && run.Conclusion != shared.StartupFailure {
		...
		runID, err = shared.PromptForRun(cs, filtered)
		if err != nil {
			return err
		}
	}
