...
func GetRunsByStatus(client *api.Client, repo ghrepo.Interface, limit int, status Status) ([]Run, error) {
	path := fmt.Sprintf("repos/%s/actions/runs?status=%s", ghrepo.FullName(repo), status)
